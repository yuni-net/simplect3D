...
namespace si3
{
	box_data::box_data(LPDIRECT3DDEVICE9 device, float roughness)
	{
		this->device = device;
		...
		const float height = diameter*pi;
		...
		*/
		...
		for (int index_y = 0; index_y < top_num_y; ++index_y)		// 上の行から下の行へ頂点情報を格納してゆく
		{
			for (int index_x = 0; index_x < top_num_x; ++index_x)	// 左の列から右の列へ頂点情報を格納してゆく
			{
				...
				vertex.pos.y = 0.0f;
				vertex.pos.z = static_cast<float>(height*(top_num_y - 1 - index_y)) / piece_num_y;
